...
const errors = errorFileFor(
  'desktop-client/src/components/modals/manager/ImportYNAB5Modal.tsx',
);

// Import failures the user can fix by picking a different file (R7).
const USER_FIXABLE_IMPORT_ERRORS = new Set(['parse-error', 'not-ynab5']);
    ...
    if (res) {
      ...
      try {
        ...
        if (USER_FIXABLE_IMPORT_ERRORS.has(err.message)) {
          errors.expected('importing a YNAB5 budget', err);
        } else {
          errors.caught('importing a YNAB5 budget', err);
        }
