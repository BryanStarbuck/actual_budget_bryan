...
import type { FoldSummary } from './fold.ts';
import { memorySink } from './test-sink.ts';

const errors = errorFileFor('error-file/src/fold.test.ts');

beforeEach(() => resetErrorFileForTests());
afterEach(() => vi.useRealTimers());

describe('fold (R10)', () => {
  it('500 identical faults in 10 s → 1 line + 1 summary (M6)', () => {
    vi.useFakeTimers();
    const sink = memorySink();
    setErrorSink(sink);
    for (let i = 0; i < 500; i++) {
      errors.caught('syncing the budget', new Error(`timeout after ${i} ms`));
      vi.advanceTimersByTime(20);
    }
    expect(sink.records).toHaveLength(1);
    vi.advanceTimersByTime(61_000);
    expect(sink.records).toHaveLength(2);
    expect(sink.records[1]?.error).toMatch(
      /^×499 more in the previous 60s \(first at \d\d:\d\d:\d\d\): Error: timeout after 499 ms$/,
    );
  });
  ...
  it('normalises digits, hex, uuids and paths so `id 123` folds with `id 456`', () => {
    ...
    expect(normalizeMessage('row 0b6e5d9a-2a8a-4d4b-9f5e-6a0c7d1f2e3b')).toBe(
      'row <uuid>',
    );
    expect(normalizeMessage('open /Users/x/a.db failed')).toBe(
      'open <path> failed',
    );
  ...
  it('the key cap evicts least-recently-seen and still writes owed summaries', () => {
    ...
    const folder = createBurstFolder({
      maxKeys: 2,
    ...
    expect(summaries).toEqual([
      { key: 'a', count: 1, windowMs: 60_000, firstAt: 0 },
    ]);
    folder.reset();
  });
  ...
  it('the stack is never part of the key', () => {
    ...
    a.stack = 'Error: same\n    at one (packages/a.ts:1:1)';
    const b = new Error('same');
    b.stack = 'Error: same\n    at two (packages/b.ts:2:2)';
