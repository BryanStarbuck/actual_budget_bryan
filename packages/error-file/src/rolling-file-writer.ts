// NODE ONLY. Ported from ~/BGit/all/app/code/packages/logging/src/rolling-file-writer.ts
// (pm/error_err.mdx §1.1, §4.3). Changes from the original:
//
//   • files are created at mode 0600 and the directory at 0700 (§3.1, §11);
//   • the first-use prepare (mkdir + stat) is ASYNC on the drain path, so no synchronous fs call is
//     made anywhere but the overflow drain, flush(), and a roll (§9);
//   • several processes append to one file, so before a roll the REAL size is checked (statSync):
//     a process that finds the file already rolled by someone else resets its cache instead of
//     rolling a second time (§4.3).
//
